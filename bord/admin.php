...
<html lang="en">
  ...
  <body style="background: url('bl3.jpeg');">
...
if (isset($_POST['login'])) {
  require 'db.php';
    ...
    $sel_user = "select * from admin where admin_name='$admin_name'";
    ...
    if($check_user > 1){
        ...
        $hashedpwd = password_verify($admin_pass, $row['admin_password']);
        if($hashedpwd == true){
            echo "<script>window.open('dashbord.php?logged_in=you have succesfully logged in', '_self')</script>";
        }
    }
...
?>
    ...
    <div class="form-group mx-3">
    <input type="submit" class="form-control btn btn-primary btn-sm" value="Login" name="login">
    </div>
